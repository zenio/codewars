 ...
 * couple of old songs and make dubstep remixes from them.
 *
 ...
 * a certain number of words "WUB" before the first word of the song (the 
 * number may be zero), after the last word (the number may be zero), and 
 * between words (at least one between any pair of neighbouring words), and 
 * then the boy glues together all the words, including "WUB", in one string 
...
function songDecoder(song){
  return song.split(/WUB/).filter(c => c).join(" ").trim();
}